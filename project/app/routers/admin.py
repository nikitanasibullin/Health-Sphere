...
def delete_schedule(
    ...
    appointments_count = db.query(models.Appointment)\
        .filter(models.Appointment.schedule_id == schedule_id)\
        .count()
    ...
    if appointments_count > 0:
        raise HTTPException(
            status_code=400,
            detail=f"Невозможно удалить расписание. С ним связано {appointments_count} записей на прием. "
                  "Сначала удалите или переназначьте записи."
        )
    
    # Сохраняем информацию для ответа
    schedule_info = f"Врач ID: {schedule.doctor_id}, дата: {schedule.date}, время: {schedule.start_time}-{schedule.end_time}"
    ...
    return {
        "message": f"Расписание успешно удалено",
