...
export function useDoctorData() {
    ...
    const getPatientContraindications = async (patientId) => {
        try {
            const report = await getPatientMedicationReport(patientId)
            if (!report) return []

            const directContras = report.direct_contraindications?.map(c => c.contradiction) || []
            const medContras = report.medicament_based_contraindications?.map(c => c.contradiction) || []

            return [...new Set([...directContras, ...medContras])]
        } catch (error) {
